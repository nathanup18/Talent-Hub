...
import { Search, MapPin, Briefcase, GraduationCap, DollarSign, Filter, X } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-card border border-card-border rounded-lg p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-6 items-center">
          <div className="bg-primary/10 rounded-lg p-4 min-w-[140px]">
            ...
            <div className="text-3xl font-mono text-primary-foreground font-semibold text-foreground">
              {breakdown?.total || 0}
            </div>
          </div>
          ...
          <div className="flex-1 flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            {breakdown?.byRoleCategory?.map((item) => (
              <div key={item.category} className="bg-muted border border-border rounded-lg px-4 py-3 min-w-[120px]">
                <div className="text-xs text-muted-foreground mb-1">{item.category}</div>
                <div className="text-xl font-mono font-medium">{item.count}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className={`w-full lg:w-64 flex-shrink-0 bg-card border border-card-border rounded-lg p-5 shadow-sm space-y-6 ${showFilters ? 'block' : 'hidden lg:block'}`}>
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Min Years Experience: <span className="font-mono">{filters.yearsExpMin || 0}</span></label>
            <input 
              type="range" 
              min="0" 
              max="20" 
        ...
        <div className="flex-1">
          {isLoading ? (
            ...
            <div className="bg-card border border-card-border rounded-lg p-12 text-center shadow-sm">
              ...
              <p className="text-muted-foreground">Try adjusting your filters or search terms to find more candidates.</p>
                  ...
                  <div className="bg-card border border-card-border hover:border-primary/50 transition-colors rounded-lg p-5 shadow-sm h-full flex flex-col cursor-pointer group">
                    ...
                    <div className="space-y-2 mb-4 flex-1">
                      ...
                      <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        <span className="font-mono">${(candidate.compRangeMin / 1000).toFixed(0)}k - ${(candidate.compRangeMax / 1000).toFixed(0)}k</span>
                      </div>
                    </div>
